...
<div class="pt-20">
  <section class="flex justify-center items-center py-16 px-4">
    <div class="bg-white text-black rounded-2xl shadow-lg w-full max-w-6xl p-8">
      ...
      <!-- Chart.js and Live Animation Script -->
      ...
      <script>
        (function () {
          ...
          const raw = @json($earning_stats['earnings_by_month']);
          const months = Array.isArray(raw) ? raw.map(r => monthNumberToName(r.month ?? r.MONTH ?? r["MONTH(created_at)"] ?? 0)) : [];
          const totals = Array.isArray(raw) ? raw.map(r => Number(r.total ?? r.TOTAL ?? r["SUM(amount)"] ?? 0)) : [];

          // Ensure 12 labels (fill missing months with 0)
          ...
          gradient.addColorStop(0, 'rgba(16, 185, 129, 0.35)'); // green-500 @ 35%
          ...
          // Lightweight live animation: smoothly nudge recent data toward a target
          function animateLive() {
            const ds = chart.data.datasets[0].data;
            for (let i = Math.max(ds.length - 4, 0); i < ds.length; i++) {
              const base = ds[i];
              const jitter = (Math.random() - 0.5) * (base * 0.02 + 25); // up to ~2% or min jitter
